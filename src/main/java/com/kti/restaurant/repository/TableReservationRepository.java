...
public interface TableReservationRepository extends JpaRepository<TableReservation, Integer> {
	@Query("select tr from TableReservation tr join RestaurantTable rt on tr.table.id = rt.id where tr.table.id = ?1 and tr.durationStart > ?2 and tr.durationStart < ?3")
